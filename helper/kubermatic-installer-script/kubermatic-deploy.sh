...
[[ $(helm version -c --short) =~ ^Client.*v2.*$ ]] && echo "helm2 detected!" || (echo "This script requires helm2! Please install helm2: https://v2.helm.sh/docs/using_helm" && exit 1)
...
#      fi
  ...
  kubermatic)
    ...
    if [[ "${1}" = "master" ]]; then
      ...
      if grep -q 'discovery_url\|oidc_issuer_url' ${VALUES_FILE}; then
        deploy "iap" "iap" ./iap/
      else
